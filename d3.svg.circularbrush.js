d3.svg.circularbrush = function() {
	...
	var _brushData = [
		...
		{ startAngle: _extent[0] - 0.2, endAngle: _extent[0], class: 'resize e' },
		{ startAngle: _extent[1], endAngle: _extent[1] + 0.2, class: 'resize w' }
	];
	var _newBrushData = [];
	...
	function _circularbrush(_container) {
		...
		_brushG.select('path.extent').on('mousedown.brush', resizeDown);

		_brushG.selectAll('path.resize').on('mousedown.brush', resizeDown);
